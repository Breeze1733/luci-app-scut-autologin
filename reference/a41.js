...
//      onlineMonitor           = parseInt(json.data.online_monitor) || 1;
        ...
        findMac                 = parseInt(json.data.find_mac) || findMac;
        ...
        storeExpireTime         = parseInt(json.data.store_expire_time) || storeExpireTime;
  ...
  checkStatus: function () {
    ...
    if(checkOnlineMethod == 1) {
      ...
      data = {
        'user_account': '', 		// 认证账号
        'user_password': '', 		// 认证密码
		'wlan_user_mac': $.trim(term.mac).toUpperCase(), //Mac需要转为大写
		'curr_user_ip': util.ipToParseInt($.trim(term.ip)), //兼容微信浏览器，点分IP转为十进制
		'curr_user_ip': util.ipToParseInt($.trim(term.ip)), //兼容微信浏览器，点分IP转为十进制IP
